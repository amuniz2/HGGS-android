...
abstract public class BaseListActivity<T extends PersistentItem> extends GroceryFileConsumer /*implements IDataAccessor*/ {
 ...
 //   abstract protected  void editItem(T item);
	...
	@Override 
	   protected void onResume()
	   {
		   ListView lv = getListView();
	    	// save the list 
		   if (lv.getFirstVisiblePosition() > _firstPos)
		   {
			   if (Build.VERSION.SDK_INT > 10)
			   {
				   lv.smoothScrollToPositionFromTop(_firstPos, 0);
			   }
			   else
			   {
				   lv.smoothScrollToPosition(_firstPos);
			   }   
		   }
		   super.onResume();
	   }
	   @Override 
	    protected void onPause()
	    {
		   ListView lv = getListView();
	    	// save the list 
		   _firstPos = lv.getFirstVisiblePosition();
	    	if (saveList() )
	    	{
	    	...
	    	if (resultCode != RESULT_CANCELED)
	        {
	    		...
	    		{		
		           	...
		           	if (!itemsToSave.contains(itemEdited))
				    {
				    	...
				    	addItemToOtherList(itemEdited, data);
				    }
				    else
				    {
				    	pos = itemsToSave.indexOf(itemEdited);
   ...
   @TargetApi(11)
    ...
    protected void addItemToOtherList(T newItem, Intent data) {}
